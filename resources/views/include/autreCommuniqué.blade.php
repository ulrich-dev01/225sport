    ...
    <div class="mb-3">
        ...
        @foreach ($communiques as $communique)
            <div class="mb-3 bg-white d-flex align-items-center" style="height: 110px; ">
                    <img class="img-fluid" src="{{ Storage::url($communique->image) }}" alt="{{ $communique->titre }}" width="100">
                    <div class="px-3 border w-100 h-100 d-flex flex-column justify-content-center border-left-0">
                        <div class="mb-2">
                            <a class="text-body" href=""><small>{{ $communique->created_at->translatedFormat('l d F Y') }}</small></a>
                        </div>
                        <a class="m-0 h6 text-secondary text-uppercase font-weight-bold" href="">{{ Str::limit(strip_tags($communique->titre), 40, '...') }}</a>
                    </div>
            </div>
        @endforeach
